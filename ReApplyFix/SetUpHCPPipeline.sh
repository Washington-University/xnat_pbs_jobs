...
local_log()
{
	...
	local date_time
	date_time=$(date)
	local tool_name="ReApplyFix/SetUpHCPPipeline.sh"
	echo "${date_time} - ${tool_name} - ${msg}"
}

local_log "Setting up for running ReApplyFix pipeline"
...
if [ "${COMPUTE}" = "CHPC" ]; then
	...
	if [ "${CLUSTER}" = "2.0" ] ; then
		...
		local_log "Set up to use ICAFIX at ${ICAFIX}"

		# MATLAB
		export MATLAB_COMPILER_RUNTIME=/export/matlab/MCR/R2016b/v91
		local_log "MATLAB_COMPILER_RUNTIME: ${MATLAB_COMPILER_RUNTIME}"
