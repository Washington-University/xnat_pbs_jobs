...
{
	local msg=${1}
	echo "SubmitPostMsmAllTaskAnalysis.OneSubject.sh: ${msg}"
}
...
get_options() 
{
	...
	inform "Notification Email: ${g_notify}"
...
main()
{
	...
	for task in ${task_scan_names} ; do
		...
		working_directory_name="${BUILD_HOME}/${g_project}/${PIPELINE_NAME}_${current_seconds_since_epoch}_${g_subject}_${task}"
