...
subject_file_name="subjectfiles/${project}.subjects"
log_Msg "Retrieviving subject list from: ${subject_file_name}"
...
for subject in ${subjects} ; do
	...
	if [[ ${subject} != \#* ]]; then
		...
		log_Msg "--------------------------------------------------------------------------------"
		log_Msg " Submitting MultiRunICAFIX job for subject: ${subject}"
		log_Msg " Using server: ${server}"
		log_Msg "--------------------------------------------------------------------------------"

		${XNAT_PBS_JOBS}/MultiRunICAFIX/SubmitMultiRunICAFIX.OneSubject.sh \
						--user=${userid} \
						--password=${password} \
						--server=${server} \
						--project=${project} \
