	...
	for scan_name in ${resting_state_scan_names} ; do
		...
		for pe_dir in ${phase_encoding_dirs} ; do
			...
			# Submit job to put the results in the DB
 			put_script_file_to_submit=${XNAT_PBS_JOBS_LOG_DIR}/${g_subject}.${scan}.IcaFixProcessingHCP.${g_project}.${g_session}.${current_seconds_since_epoch}.XNAT_PBS_PUT_job.sh
