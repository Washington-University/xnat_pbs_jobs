...
project="HCP_Staging"
subject_file_name="${SUBJECT_FILES_DIR}/${project}.IcaFixProcessingHCP.subjects"
...
for subject in ${subjects} ; do
	...
	if [[ ${subject} != \#* ]]; then
		...
		echo ""
		echo "--------------------------------------------------------------------------------"
		echo " Submitting IcaFixProcessingHCP jobs for subject: ${subject}"
...
#		at now + ${delay} minutes <<EOF 
		...
		${HOME}/pipeline_tools/xnat_pbs_jobs/IcaFixProcessingHCP/SubmitIcaFixProcessingHCP.OneSubject.sh \
			--user=${userid} \
			--password=${password} \
			--put-server=${server} \
			--project=${project} \
