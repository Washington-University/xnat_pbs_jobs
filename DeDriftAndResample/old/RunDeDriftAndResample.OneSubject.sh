...
SCRIPT_NAME="RunDeDriftAndResample.OneSubject.sh"
HCP_ROOT="/HCP"
...
main()
{
	...
	working_directory_name="${BUILD_HOME}/${g_project}/DeDriftAndResample.${g_subject}.${current_seconds_since_epoch}"
	...
	script_file_to_submit=${working_directory_name}/${g_subject}.DeDriftAndResample.${g_project}.${g_session}.${current_seconds_since_epoch}.XNAT_PBS_job.sh
	if [ -e "${script_file_to_submit}" ]; then
		rm -f "${script_file_to_submit}"
	fi

	touch ${script_file_to_submit}
	echo "${HOME}/pipeline_tools/xnat_pbs_jobs/DeDriftAndResample/DeDriftAndResample.XNAT.sh \\" >> ${script_file_to_submit}
	...
	standard_out_file=${working_directory_name}/${g_subject}.DeDriftAndResample.${g_project}.${g_session}.${current_seconds_since_epoch}.interactive.stdout
	standard_err_file=${working_directory_name}/${g_subject}.DeDriftAndResample.${g_project}.${g_session}.${current_seconds_since_epoch}.interactive.stderr
	...
	put_script_file_to_submit=${working_directory_name}/${g_subject}.DeDriftAndResample.${g_project}.${g_session}.${current_seconds_since_epoch}.XNAT_PBS_PUT_job.sh
	...
	echo "  --resource-suffix=\"${MSM_ALL_DEDRIFT_RESOURCE_NAME}\" \\" >> ${put_script_file_to_submit} 
	echo "  --reason=\"DeDriftAndResample\" " >> ${put_script_file_to_submit}
	
	chmod +x ${put_script_file_to_submit}

	standard_out_file=${working_directory_name}/${g_subject}.DeDriftAndResample.${g_project}.${g_session}.${current_seconds_since_epoch}.interactive_PUT.stdout
	standard_err_file=${working_directory_name}/${g_subject}.DeDriftAndResample.${g_project}.${g_session}.${current_seconds_since_epoch}.interactive_PUT.stderr
