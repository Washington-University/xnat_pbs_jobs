...
#
# After all the per subject individual package reports are generated by submitting the 
# CheckPackageExistenceHCP.sh script, then they all can be concatenated into a single 
# package report for the entire project by a command like:
#
#   cat *.tsv > All_HCP900_subjects.PackageReport.tsv
# 
# Then this script is intended to be used to split that single file into separate 
# Tab Separated Values (.tsv) files by the package type instead of by the subject id.
# 
