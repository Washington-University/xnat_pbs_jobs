...
{
	local msg=${1}
	echo "TaskAnalysisPackaging.sh: ${msg}"
}
...
get_options()
{
	...
	if [ -z "${g_series}" ]; then
		inform "ERROR: series (--series=) required"
		error_count=$(( error_count + 1 ))
	else
		inform "g_series: ${g_series}"
	fi
...
main()
{
	...
	pkg_cmd+=" --outputType ANALYSIS"   # [UNPROC | PREPROC | ANALYSIS | FIX]
	pkg_cmd+=" --packet FUNCTIONAL"    # [STRUCTURAL | FUNCTIONAL | DIFFUSION]
	pkg_cmd+=" --series ${g_series}"
