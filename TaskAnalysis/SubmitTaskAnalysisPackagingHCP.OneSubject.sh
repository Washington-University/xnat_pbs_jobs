...
main()
{
	...
	for series in tfMRI_EMOTION tfMRI_GAMBLING tfMRI_LANGUAGE tfMRI_MOTOR tfMRI_RELATIONAL tfMRI_SOCIAL tfMRI_WM ; do

		sleep 5s
		current_seconds_since_epoch=`date +%s`
		working_directory_name="${BUILD_HOME}/${g_project}/TaskAnalysisPackagingHCP.${series}.${current_seconds_since_epoch}_${g_subject}"
		...
		script_file_to_submit=${working_directory_name}/${g_subject}.${series}.TaskAnalysisPackagingHCP.XNAT_PBS_job.sh
		...
		echo "/home/HCPpipeline/pipeline_tools/xnat_pbs_jobs/TaskAnalysis/TaskAnalysisPackaging.sh \\" >> ${script_file_to_submit}
