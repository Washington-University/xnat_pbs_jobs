...
main()
{
	...
	for resting_state_scan_dir in ${resting_state_scan_dirs} ; do
		...
		scan_name=${scan_name%%_${NEGATIVE_PHASE_ENCODING_DIRECTION}}
		scan_name=${scan_name%%_${POSITIVE_PHASE_ENCODING_DIRECTION}}
	...
	for scan_name in ${resting_state_scan_names} ${task_scan_names} ; do
		...
		for postfix in ${POSITIVE_PHASE_ENCODING_DIRECTION} ${NEGATIVE_PHASE_ENCODING_DIRECTION} ; do
			...
			working_directory_name="${BUILD_HOME}/${g_project}/FunctionalPreprocessingHCP7T_${current_seconds_since_epoch}_${g_subject}_${scan}"
			...
			echo "${XNAT_PBS_JOBS_HOME}/7T/FunctionalPreprocessingHCP/FunctionalPreprocessingHCP7T.XNAT.sh \\" >> ${script_file_to_submit}
			...
			echo "  --structural-ref-session=\"${g_subject}_3T\" \\" >> ${script_file_to_submit}
