...
inform() 
{
	echo "CreatePreprocExtendedPackage.sh: ${1}"
}
...
main()
{
	...
	for modality in MOVIE REST RET ; do
		...
		new_package_name="${g_subject}_7T_${modality}_preproc_extended.zip"
		...
		if [ "${modality}" = "RET" ]; then
			...
			retinotopy_scans=""
			retinotopy_scans+=" RETCCW_7T_AP "
			retinotopy_scans+=" RETCW_7T_PA "
			retinotopy_scans+=" RETEXP_7T_AP "
			retinotopy_scans+=" RETCON_7T_PA "
			retinotopy_scans+=" RETBAR1_7T_AP "
			retinotopy_scans+=" RETBAR2_7T_PA "
			
			for retinotopy_scan in ${retinotopy_scans} ; do

				file_list+=" MNINonLinear/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}.L.native.func.gii "
				file_list+=" MNINonLinear/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}.nii.gz "
				file_list+=" MNINonLinear/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}.R.native.func.gii "

				file_list+=" T1w/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}_dropouts.nii.gz "
				file_list+=" T1w/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}_sebased_bias.nii.gz "
				file_list+=" T1w/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}_sebased_reference.nii.gz "
			...
			for scan_dir in ${scan_dirs} ; do
				...
				file_list+=" MNINonLinear/Results/${fMRIName}/${fMRIName}.L.native.func.gii "
				file_list+=" MNINonLinear/Results/${fMRIName}/${fMRIName}.nii.gz "
				file_list+=" MNINonLinear/Results/${fMRIName}/${fMRIName}.R.native.func.gii "
		...
		inform ""
		release_notes_file=${script_tmp_dir}/${g_subject}/release-notes/${g_subject}_7T_${modality}_preproc_extended.txt
