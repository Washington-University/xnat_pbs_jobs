	...
	for modality in MOVIE REST RET ; do
		...
		if [ "${modality}" = "RET" ]; then
			...
			for retinotopy_scan in ${retinotopy_scans} ; do
				file_list+=" MNINonLinear/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}_Atlas_1.6mm_hp2000_clean.dtseries.nii "
				file_list+=" MNINonLinear/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}_Atlas_1.6mm_MSMAll_hp2000_clean.dtseries.nii "

				fix_readme_file=${script_tmp_dir}/${g_subject}_full/MNINonLinear/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}_Atlas_1.6mm_hp2000_clean.README.txt
				
				touch ${fix_readme_file}
				echo "tfMRI_${retinotopy_scan}_Atlas_1.6mm_hp2000_clean.dtseries.nii was generated by applying \"multi-run FIX\" ('hcp_fix_multi_run')" >> ${fix_readme_file}
				echo "across the following individual runs:" >> ${fix_readme_file}
				for rscan in ${retinotopy_scans} ; do
					echo "  ${rscan}" >> ${fix_readme_file}
				done

				file_list+=" MNINonLinear/Results/tfMRI_${retinotopy_scan}/tfMRI_${retinotopy_scan}_Atlas_1.6mm_hp2000_clean.README.txt "
			done
		...
		inform ""
		release_notes_file=${script_tmp_dir}/${g_subject}/release-notes/${g_subject}_7T_${modality}_1.6mm_fix.txt
