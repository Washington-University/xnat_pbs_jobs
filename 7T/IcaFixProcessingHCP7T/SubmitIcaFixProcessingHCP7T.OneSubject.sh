...
{
	local msg=${1}
	echo "SubmitIcaFixProcessingHCP7T.OneSubject.sh: ${msg}"
}
...
main()
{
	...
	# Determine what preprocessed resting state scans are available for the subject
	pushd ${DATABASE_ARCHIVE_ROOT}/${g_project}/arc001/${g_session}/RESOURCES

	resting_state_scan_names=""
	resting_state_scan_dirs=`ls -d ${RESTING_STATE_FMRI_PREFIX}_*_${PREPROCESSED_SUFFIX}`
	for resting_state_scan_dir in ${resting_state_scan_dirs} ; do
		scan_name=${resting_state_scan_dir%%_${PREPROCESSED_SUFFIX}}
	...
	inform "Preprocessed resting state scans available for subject: ${resting_state_scan_names}"

	# Determine what preprocessed task scans are available for the subject
	pushd ${DATABASE_ARCHIVE_ROOT}/${g_project}/arc001/${g_session}/RESOURCES
	
	task_scan_names=""
	task_scan_dirs=`ls -d ${TASK_FMRI_PREFIX}_*_${PREPROCESSED_SUFFIX}`
	for task_scan_dir in ${task_scan_dirs} ; do
		scan_name=${task_scan_dir%%_${PREPROCESSED_SUFFIX}}
	...
	inform "Preprocessed task scans available for subject: ${task_scan_names}"

	# Determine what preprocessed MOVIE scans are available for the subject
	pushd ${DATABASE_ARCHIVE_ROOT}/${g_project}/arc001/${g_session}/RESOURCES

	movie_scan_names=""
	movie_scan_dirs=`ls -d *MOVIE*_${PREPROCESSED_SUFFIX}`
	for movie_scan_dir in ${movie_scan_dirs} ; do
		scan_name=${movie_scan_dir%%_${PREPROCESSED_SUFFIX}}
		scan_name=${scan_name##${TASK_FMRI_PREFIX}_}
		movie_scan_names=${movie_scan_names//$scan_name/}
		movie_scan_names+=" ${scan_name}"
	done

	popd

	inform "Preprocessed movie scans available for subject: ${movie_scan_names}"
	...
	for scan_name in ${scan_list} ; do
		...
		echo "#PBS -l nodes=1:ppn=1,walltime=24:00:00,mem=40gb,vmem=55gb" >> ${script_file_to_submit}
