...
subject_file_name="${SUBJECT_FILES_DIR}/DeDriftAndResampleHCP7T.subjects"
...
for subject_spec in ${subjects} ; do
	...
	if [[ ${subject_spec} != \#* ]]; then
		...
		inform ""
		inform "--------------------------------------------------------------------------------"
		inform " Submitting DeDriftAndResampleHCP7T jobs for:"
		...
		${HOME}/pipeline_tools/xnat_pbs_jobs/7T/DeDriftAndResampleHCP7T/SubmitDeDriftAndResampleHCP7T.OneSubject.sh \
			--user=${userid} \
			--password=${password} \
			--put-server=${server} \
			--project=${project} \
			...
			--setup-script=${SCRIPTS_HOME}/SetUpHCPPipeline_DeDriftAndResampleHCP7T.sh
