...
get_options() 
{
	...
	if [ -z "${g_ref_project}" ]; then
		g_ref_project="HCP_500"
	fi
	echo "Structural Reference Project: ${g_ref_project}"
...
main()
{
	...
	working_directory_name="${BUILD_HOME}/${g_project}/DiffusionPackagingHCP7T.${g_subject}.${current_seconds_since_epoch}"
	...
	script_file_to_submit=${working_directory_name}/${g_subject}.DiffusionPackagingHCP7T.XNAT_PBS_job.sh
	...
	echo "${XNAT_PBS_JOBS}/7T/DiffusionPreprocessingHCP7T/DiffusionPackagingHCP7T.sh \\" >> ${script_file_to_submit}
	echo "  --project=${g_project} \\" >> ${script_file_to_submit}
	echo "  --ref-project=${g_ref_project} \\" >> ${script_file_to_submit}
