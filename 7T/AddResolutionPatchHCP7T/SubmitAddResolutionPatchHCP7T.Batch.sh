...
subject_file_name=${SUBJECT_FILES_DIR}/AddResolutionPatchHCP7T.subjects
...
for project_subject in ${subjects} ; do
	...
	if [[ ${project_subject} != \#* ]]; then
		...
		server="db-shadow${shadow_number}.nrg.mir:8080"
 		...
 		inform ""
		inform "--------------------------------------------------------------------------------"
		inform " Submitting AddResolutionPatchHCP7T Preprocessing job for:"
		inform "   project: ${project}"
		inform "   subject: ${subject}"
		inform "    server: ${server}"
		inform "--------------------------------------------------------------------------------"

		${HOME}/pipeline_tools/xnat_pbs_jobs/7T/AddResolutionPatchHCP7T/SubmitAddResolutionPatchHCP7T.OneSubject.sh \
			--user=${userid} \
			--password=${password} \
			--project=${project} \
			--subject=${subject} \
