...
local_log()
{
	...
	local date_time
	date_time=$(date)
	local tool_name="SetUpHCPPipeline.sh"
	echo "${date_time} - ${tool_name} - ${msg}" 
}

local_log "Setting up for running MSMAll Registration pipeline"
local_log "This script must be SOURCED to correctly set up the environment"
...
if [ "$COMPUTE" = "CHPC" ] ; then
	...
	if [ "${CLUSTER}" = "2.0" ] ; then
		...
		local_log "Processing set up for cluster ${CLUSTER} is currently not supported."
	...
	local_log "Processing set up for ${COMPUTE} is currently not supported."
	local_log "EXITING WITH NON-ZERO EXIT STATUS (UNSUCCESSFUL EXECUTION)"
	exit 1
