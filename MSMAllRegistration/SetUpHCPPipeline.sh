...
log "Setting up for running MSMAll Registration pipeline"
log "This script must be SOURCED to correctly set up the environment"
...
if [ "$COMPUTE" = "CHPC" ] ; then
	...
	if [ "${CLUSTER}" = "2.0" ] ; then
		...
		log "Processing set up for cluster ${CLUSTER} is currently not supported."
	...
	log "Processing set up for ${COMPUTE} is currently not supported."
	log "EXITING WITH NON-ZERO EXIT STATUS (UNSUCCESSFUL EXECUTION)"
	exit 1
